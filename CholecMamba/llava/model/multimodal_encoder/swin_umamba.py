...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        y = self.out_norm(y.to(target_dtype))
        y = y * F.silu(z)
        out = self.out_proj(y)
...
class VSSMEncoder(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, depths=[2, 2, 9, 2], 
                 dims=[128, 256, 512, 1024], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2,
...
class SwinUMamba(nn.Module):
    ...
    def forward(self, x_in):
        ...
        #print("vss_outs0 shape:", vss_outs[0].shape)
        #print("vss_outs1 shape:", vss_outs[1].shape)
        #print("vss_outs2 shape:", vss_outs[2].shape)
        #print("vss_outs3 shape:", vss_outs[3].shape)
        #print("vss_outs4 shape:", vss_outs[4].shape)
...
class VCMamba(nn.Module):
    def __init__(self): 
        ...
        self.stem[1].weight = nn.Parameter(torch.ones(64))
        self.stem[1].bias = nn.Parameter(torch.zeros(64))
        self.vssm_encoder = VSSMEncoder(patch_size=2, in_chans=64)
    def forward(self, x):
        x = self.stem(x)
        x = self.vssm_encoder(x)
        return x
    
class VMambaImageProcessor:
    def __init__(self, target_size=(256, 256), resample_method=Image.BICUBIC, crop_size = 256):
        self.target_size = target_size
        self.resample_method = resample_method
        self.crop_size = crop_size

    def preprocess(self, image, return_tensors='np'):
        """
        ...
        """
        # Resize the image
        image = image.resize(self.target_size, resample=self.resample_method)
        ...
        if return_tensors == 'pt':
            # Convert to PyTorch tensor and set dtype to float32
            image = torch.as_tensor(image.copy()).to(dtype=torch.float32).contiguous()
        elif return_tensors == 'np':
            # Ensure the image is a NumPy array with dtype float32
            image = image.astype(np.float32)
        else:
            raise ValueError(f"Unsupported return_tensors value: {return_tensors}. Use 'np' or 'pt'.")
